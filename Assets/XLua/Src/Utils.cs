...
namespace XLua
{
	...
	public static partial class Utils
	{
		...
		//填充Type对应的几个table信息, 并给函数 item_getter, item_setter赋值
		...
		//相当于执行了 BeginObjRegister, EndObjRegister, BeginClassRegister, EndClassRegister,
		...
		//对类的非静态值(例如成员变量，成员方法等)进行注册前做一些准备工作
		//push四个table, 每个idx对应一个table,当然,如果对应的count为0, idx对应的是一个nil
			...
			int setter_count, int type_id = -1)
		{
			...
			{
				LuaAPI.luaL_getmetatable(L, type.FullName);//一般都可以获取到, 因为在ObjectTranslator.TryDelayWrapLoader()函数中，对于每个type对应的fullName都会自动生成一个表格
				if (LuaAPI.lua_isnil(L, -1))
				{
					LuaAPI.lua_pop(L, 1);
					LuaAPI.luaL_newmetatable(L, type.FullName);
		...
		{
			//注意这里的top的值为lua_gettop{栈顶元素的索引,而不是L->top},指向的是栈顶的有效元素
			//栈底为1，栈顶为-1，返回idx在栈中的位置（正序，从栈底算起）
...
#else
		//结束对类的非静态值的注册
		//设置表OBJ_META_IDX(obj_meta)中的"__index"和"__newindex", 并将BeginObjectRegister()中push到栈中的四个table 出栈
...
#endif
		{
			...
			LuaAPI.gen_obj_indexer(L);//新建一个cclosure,
			...
			LuaAPI.lua_pop(L, 4);//将BeginObjectRegister()中push到栈中的四个table 出栈
		}
...
#else
		//对类的静态值(例如静态变量，静态方法等)进行注册前做一些准备工作
		//1、push 四个table, 栈底开始依次为: cls_filed, cls_meta, cls_getter, cls_setter
		//2、根据type.fullName,建立整个table链
		...
		//结束对类的静态值的注册
		//为CLS_IDX(cls_field)的元表CLS_META_IDX(cls_meta)设置__index元方法和__nexindex元方法
		public static void EndClassRegister(Type type, RealStatePtr L, ObjectTranslator translator)
		{
			...
			LuaAPI.lua_pop(L, 4);//将BeginClassRegister()函数中push到栈中的四个table全部弹出
		}

		//Type(UnityEngine.Vector3), 返回list<string>{"UnityEngine", "Vector3"}
		...
		//Type=UnityEngine.Vector3时, 从表格register["xlua_csharp_namespace"/"CS"]中开始,一步一步,建立整个表格链
		//register["xlua_csharp_namespace"/"CS"] = "UnityEngine"_t 表格
		//"UnityEngine"_t["Vector3"] = "Vector3"_t 表格
		public static void SetCSTable(RealStatePtr L, Type type, int cls_table)//注意, cls_table对应的表格为 Utils.CLS_IDX_t(cls_field)
		{
			int oldTop = LuaAPI.lua_gettop(L);
			...
			//注意这里的循环条件 "i < path.count -1", 如果是UnityEngine.Vector3,则检测到"UnityEngine"后就会退出循环
			for (int i = 0; i < path.Count - 1; ++i)
			{
				...
				if (LuaAPI.lua_isnil(L, -1))//如果注册表[xlua_csharp_namespace]中没有key path[i],则添加一个path[i]={}键值对
				{
					LuaAPI.lua_pop(L, 1);
				...
				LuaAPI.lua_remove(L, -2);
			}//假设path为 "UnityEngine.Vectro3", 则for循环完成后, 栈顶为"UnityEngine"_table,即table(path[len - 2])
