...
namespace XLua
{
    ...
    public class Hotfix
    {
            ...
            foreach (var method in type.Methods)
            {
                ...
                if (method.Name != ".cctor" && !method.IsAbstract && !method.IsPInvokeImpl && method.Body != null && !method.Name.Contains("<"))
                {
                    ...
                    if ((isInline || method.HasGenericParameters || genericInOut(method, hotfixType)) 
                        ? !injectGenericMethod(method, hotfixType) ://泛型方法
                        !injectMethod(method, hotfixType))//普通方法
                    {
                        return false;
                    }
                }
        ...
        static private string GetPdbPathFromDllPath(string dllPath)
        {
            if (dllPath.EndsWith(".dll"))
            {
                return dllPath.Substring(0, dllPath.Length - 4) + ".pdb";
            }
            return null;
        }
...
#else
            string pdbFilePath = GetPdbPathFromDllPath(assemblyPath);
            
            if (pdbFilePath != null && File.Exists(pdbFilePath))
            {
        ...
        static string getDelegateName(MethodDefinition method)
        {
            ...
            //xzxtodo
            string ans = string.Format("{0} = {1}", method.FullName, luaDelegateName);
