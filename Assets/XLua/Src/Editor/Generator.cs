...
namespace CSObjectWrapEditor
{
    ...
    public static class Generator
    {
        ...
        class MethodInfoSimulation
        {
            ...
            public HashSet<string> mFuncNameInCS = new HashSet<string>();
        }
        ...
        static MethodInfoSimulation makeMethodInfoSimulation(MethodInfo method)
        {
            ...
            MethodInfoSimulation ans = new MethodInfoSimulation()
            {
            ...
            ans.mFuncNameInCS.Add(GetFuncNameInCS(method));
        ...
        static string GetFuncNameInCS(MethodBase hotfixMethod)
        {
            string methodName = hotfixMethod.Name;
            string clsName = hotfixMethod.DeclaringType.FullName;
            
            ParameterInfo[] pinfos = hotfixMethod.GetParameters();
            string sParam = "(";
            for (int i = 0; i < pinfos.Length; i++)
            {
                string pname = pinfos[i].Name;
                string pTname = pinfos[i].ParameterType.Name; //pinfos[i].ParameterType.FullName;

                if (i == pinfos.Length - 1)
                    sParam += string.Format("{0} {1}", pTname, pname);
                else
                    sParam += string.Format("{0} {1}, ", pTname, pname);
            }
            ...
            string s = string.Format("xzx makeHotfixMethodInfoSimulation-- {0}.{1}{2}", clsName, methodName, sParam);
        ...
        static MethodInfoSimulation makeHotfixMethodInfoSimulation(MethodBase hotfixMethod, HotfixFlag hotfixType)
        {
            ...
            ans.mFuncNameInCS.Add(GetFuncNameInCS(hotfixMethod));
        ...
        class MethodInfoSimulationComparer : IEqualityComparer<MethodInfoSimulation>
        {
            public bool Equals(MethodInfoSimulation x, MethodInfoSimulation y)
            {
                ...
                bool b = lastPos < 0 || xParams[lastPos].IsParamArray == yParams[lastPos].IsParamArray;
                if(b == true)
                {
                    //合并一下mFuncNameInCS
                    x.mFuncNameInCS.UnionWith(y.mFuncNameInCS);
                    y.mFuncNameInCS.UnionWith(x.mFuncNameInCS);
                }
        ...
        static void GenDelegateBridge(IEnumerable<Type> types, string save_path, IEnumerable<Type> hotfix_check_types)
        {
            ...
            {
                string gname = "__Gen_Hotfix_Delegate" + i;
                hotfxDelegates[i].DeclaringTypeName = gname;

                Debug.LogError("---------------------------------" + gname);
                int k = 0;
                foreach (string funcName in hotfxDelegates[i].mFuncNameInCS)
                {
                    k++;
                    Debug.LogError(k + ": " + funcName + '\n');
                }
            }
            ...
            //groupBy, 使用comparer来对比,将所有的hotfxDelegates进行分组, 相同的放在一组里

            int kvi = 0;
            foreach (var kv in delegates_groups)
            {
                MethodInfoSimulation m = kv.Key;

                Debug.LogError(kvi + "--------------------------------- 000000");
                kvi++;
                foreach (string funcName in m.mFuncNameInCS)
                {
        ...
        public static List<Type> GCOptimizeList = null;//中有[GCOptimize]属性的类, 查询"[GCOptimize"即可知道是哪些类
