...
namespace XLua
{
    ...
    public class LuaEnv : IDisposable
    {
        ...
        // rawL = LuaAPI.luaL_newstate(); 一个int值, 表示指针地址, 指针类型为 lua_State *
        ...
        public LuaEnv()
        {
...
#endif
            {
                ...
                //Init Base Libs, 添加一些基础库
                LuaAPI.luaopen_xlua(rawL);//这里会添加一个global table "xlua"
                ...
                translator.OpenLib(rawL);//填充global table "xlua"
                ...
                translator.Get(rawL, -1, out _G);//获取LuaTable _G
        ...
        private string init_xlua = @" 
            ...
            local import_type = xlua.import_type --xlua.xxx, 见ObjectTranslator.OpenLib()函数
            local import_generic_type = xlua.import_generic_type
            local load_assembly = xlua.load_assembly

            --注意,是t[key]或t.key找不到的时候,才使用这个__index函数, 主要是查找比如'CS.UnityEngine'中的UnityEngine{namespace},
            --查找特定类中的成员比如'CS.UnityEngine.xxxSystem.property'中的property的时候,是不会走到这里来的,在查表的时候会直接找到的
            function metatable:__index(key) 
                local fqn = rawget(self,'.fqn')
                fqn = ((fqn and fqn .. '.') or '') .. key

                --注意, fqn不含有前缀'CS.',因为本身就是在表格CS{}中去查找的
                print('xzx __index, fqn = '..fqn..', key = '..key)

                local obj = import_type(fqn)--这里面会真正的去import这个type(ObjectTranslator.importTypeFunction)

                if obj == nil then --一般是针对'CS.UnityEngine'中的UnityEngine{namespace}, 这个是没有type的,此时给new 一个metatable
                    -- It might be an assembly, so we load it too.
                    --如果为空, 有可能这个字段还是类名的一部分,那么创建一个table记录, 然后缓存返回
                    print('xzx __index, obj == nil')
                    obj = { ['.fqn'] = fqn } -- obj['.fqn']里面存储着该obj的完整typeName
                ...
                -- Cache this lookup, 缓存一下
                rawset(self, key, obj)--注意，这里将该obj设置到了self中(key, obj),这里的key并不是obj的完整typeName

                print('xzx __index return')
                return obj
            end
            ...
            xlua.hotfix = function(cs, field, func)
                ...
                for k, v in pairs(tbl) do
                    ...
                    --xlua.access对应函数 StaticLuaCallbacks.XLuaAccess
                    xlua.access(cs, cflag .. '__Hotfix0_'..k, f) -- at least one
                    --尝试给重载方法也添加上function,如果有重载的话
                    pcall(function()
                        for i = 1, 99 do
            ...
            xlua.hotfixT = function(cs, field, func, n)
                if func == nil then func = false end
                ...
                for k, v in pairs(tbl) do
                    ...
                    --xlua.access对应函数 StaticLuaCallbacks.XLuaAccess
                    xlua.access(cs, cflag .. '__Hotfix'..n..'_'..k, f)
                end
