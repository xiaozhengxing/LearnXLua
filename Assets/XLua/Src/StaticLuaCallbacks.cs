    ...
    public partial class StaticLuaCallbacks
    {
        ...
        static int FixCSFunction(RealStatePtr L)
        {
            try
            {
                ...
                //为什么这里可以使用upvalue？
                //此函数是在csharp_function_wrap中直接使用函数指针调用的 fn(L), 相当于一行代码, 当然可以使用csharp_function_wrap中的upvalue


                //这里的idx对应函数ObjectTranslator.PushFixCSFunction()中push的参数，
                //值为 List<LuaCSFunction> ObjectTranslator.fix_cs_functions的索引
        ...
        public static int ImportType(RealStatePtr L)
        {
            try
            {
                ...
                UnityEngine.Debug.Log("xzx ----------------cs--------------- ImportType: " + className);
                
                Type type = translator.FindType(className);//在各个Assembly里面查找有没有该类, 注意查找命名空间(如UnityEngine)会返回null
                if (type != null)
                {
                    if (translator.GetTypeId(L, type) >= 0)//查找到了该类, 则对其进行注册
                    {
