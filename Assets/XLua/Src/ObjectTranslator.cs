...
#pragma warning restore 414
    ...
    public partial class ObjectTranslator
	{
        ...
        //延迟加载, generate code 生成的文件的类的注册函数(如:UnityEngineVector3Wrap.__Register), 会保存到该dic中
        ...
        //aliasCfg[System.MonoType] = System.Type
        ...
        //将该类型反射相关的8个表格全部建立好并填充,两种方式:1、通过generate code方式的, 使用其"__Register()"方法, 2、其他的使用通用反射方式
        ...
        public int cacheRef;//Lua 侧缓存表的索引
        ...
        public ObjectTranslator(LuaEnv luaenv,RealStatePtr L)
		{
            ...
            LuaAPI.xlua_pushasciistring(L, "v");//__mode='v' 表示表格的value为弱引用{__mode='k'则key为弱引用, __mode='kv'则key,value都是弱引用}
        ...
        Delegate getDelegate(DelegateBridgeBase bridge, Type delegateType)
        {
            ...
            {
                // get by parameters
                MethodInfo delegateMethod = delegateType.GetMethod("Invoke");//注意这里的delegateType是一个Type(Delegate)
        ...
        public object CreateDelegateBridge(RealStatePtr L, Type delegateType, int idx)
        {
            ...
            {
                //idx对应的不是一个lua function来着的吗,怎么可以tointeger?
                //感觉和函数指针类似, 每个lua function其实就是一个指令偏移地址, tointerger之后的值都不一样
            ...
            LuaAPI.lua_pushvalue(L, idx);
            int reference = LuaAPI.luaL_ref(L);//registry_t[reference] = idx值(lua_func)
            LuaAPI.lua_pushvalue(L, idx);
            LuaAPI.lua_pushnumber(L, reference);
            LuaAPI.lua_rawset(L, LuaIndexes.LUA_REGISTRYINDEX);//registry_t[idx值(lua_func)] = reference
            ...
            if(is_delegate)
            {
                ...
                else
                {
                    ...
                    if (LuaAPI.lua_type(L, -1) == LuaTypes.LUA_TNUMBER && LuaAPI.xlua_tointeger(L, -1) == reference) //
                    {
                        ...
                        LuaAPI.lua_pushnil(L);
                        LuaAPI.lua_rawset(L, LuaIndexes.LUA_REGISTRYINDEX); // LUA_REGISTRYINDEX[func] = nil, {因为设置了nil, 所以在lua底层的gc过程中会被gc掉这个key(func)值?}
                    }
		...
		{
            if (0 != LuaAPI.xlua_getglobal(L, "xlua"))//LuaAPI.luaopen_xlua中添加"xlua"全局table
            {
        ...
        //在registry中建立typeof(LuaCSFunction)对应的table
		internal void createFunctionMetatable(RealStatePtr L)
		{
            ...
            LuaAPI.xlua_rawseti(L, -2, 1);//t[1] = type_id
            LuaAPI.lua_pop(L, 1);
        ...
        //将object o 保存到对象池ObjectTranslator.objects中, 返回索引
        ...
        //该type如果已注册过, 则直接返回type_id; 其中register[type_id] = 表格OBJ_META_IDX_t(obj_meta)
        //如果没有注册过, 则通过反射生成其对应的各个元表, 再返回type_id
        internal int getTypeId(RealStatePtr L, Type type, out bool is_first, LOGLEVEL log_level = LOGLEVEL.WARN)
        {
            ...
            if (!typeIdMap.TryGetValue(type, out type_id)) // no reference
            {
                ...
                if (LuaAPI.lua_isnil(L, -1)) //no meta yet, try to use reflection meta
                {
                    ...
                    {
                        //获取类型对应的表格 OBJ_META_IDX_t (obj_meta)
                        LuaAPI.luaL_getmetatable(L, alias_type == null ? type.FullName : alias_type.FullName);
                    }
                    else
                    {
        ...
        /*
         * 1、将obj push到栈中时, 包装成UserData{ int}, 其中int存储的值为key{ 索引, 对象为OjbectTranslator.Objects[key]}
        并赋值UserData.metatable = register[type_id],即表格OBJ_META_IDX_t(obj_meta)
         * 2、将obj 保存到对象池ObjectTranslator.objects中
        */
        public void Push(RealStatePtr L, object o)
        {
            ...
            //并赋值 UserData.metatable = register[type_id],即表格OBJ_META_IDX_t(obj_meta)
        ...
        //获取lua栈中index索引处对应的object
        //1 {index索引处保存的可能是个udata(*udata保存的值为key, 对应的对象为 ObjectTranslator.Objects[key])}
        //2 index 索引处可能不是一个 object, 而是一个数值
        ...
        internal void PushFixCSFunction(RealStatePtr L, LuaCSFunction func)
        {
            ...
            else
            {
                ...
                //注意这里调用lua_pushstdcallcfunction有一个参数，就是上面push的fix_cs_functions.Count，其实就是一个索引
                //FixCSFunctoinWrapper 就是 StaticLuaCallbacks.FixCSFunction
